...
namespace AffineTransformationsIn3D.Geometry
{
    public static class Models
    {
		...
		public static Mesh CubeWithNormals(double size)
		{
			double s = size / 2;
        ...
        public static Mesh CubeWithTexture(double size)
        {
            double s = size / 2;
			...
			double AngleY = Math.PI / 2, double AngleZ = Math.PI / 4)
        {
			...
			int[][] del_y = FloatHorizon(vertices, indices, nz, nx);
        ...
        private static int[][] FloatHorizon(Vector[] vertices, int[][] indices, int nz, int nx)
        {
